...
--------------------------------------------------------------------------------------------------------------------------------
游戏房间的设计
game_room.html (这个页面就是匹配成功之后，要跳转到的新页面)

棋盘区域，需要基于 canvas 进行实现. canvas是HTML5 新标签，可以在画布上画画
此处网格线，棋盘和棋子都是画上去的。canvas标签，有js的canvas api，实现画画的效果
...
MatcherAPI 和 GameAPI 里，游戏大厅和游戏房间都要判定，用户是否多开。
